...
_USB_BACKEND = _libusb0.get_backend()  # uses C:\Windows\System32\libusb0.dll installed by Zadig
...
class DFU(object):
    ...
    def readprotect(self):
        ...
        # Write the new block.
        # self._device.ctrl_transfer(0x21, int(Request.DNLOAD), 2, 0, [0xFF, 0xAA, 0x00, 0x55])
    ...
    def upload(self, block_number, length):
        ...
        # data = self._device.ctrl_transfer(0xA1, int(Request.UPLOAD), block_number, 0, length)
        data = self._device.ctrl_transfer(bmRequestType=0xA1,
                                          bRequest=int(Request.UPLOAD),
                                          wValue=block_number,
                                          wIndex=0,
    ...
    def get_status(self):
        status_packed = self._device.ctrl_transfer(0xA1, int(Request.GETSTATUS), 0, 0, 6)
    ...
    def clear_status(self):
        self._device.ctrl_transfer(0x21, int(Request.CLRSTATUS), 0, 0, None)

    def get_state(self):
        state_packed = self._device.ctrl_transfer(0xA1, int(Request.GETSTATE), 0, 0, 1)
        return State.map[struct.unpack('<B', state_packed)[0]]

    def abort(self):
        self._device.ctrl_transfer(0x21, int(Request.ABORT), 0, 0, None)
...
def init_dfu(idVendor=stm32_vendor, idProduct=stm32_product):
    dev = usb.core.find(idVendor=idVendor, idProduct=idProduct, backend=_USB_BACKEND)
    if dev is None:
...
def main():
    if len(sys.argv) > 1:
        if sys.argv[1] == 'read':
            ...
            dfu = init_dfu()
            firmware = data
            target_offset = int(sys.argv[3], 16)

            download(dfu, firmware, target_offset)
