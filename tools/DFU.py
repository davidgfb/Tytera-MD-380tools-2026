...
class DFU(object):
    ...
    def set_address(self, address):
        ...
        c = (address >> 16) & 0xFF
        d = (address >> 24) & 0xFF
        self._device.ctrl_transfer(0x21, int(Request.DNLOAD), 0, 0, [0x21, a, b, c, d])
    ...
    def erase_block(self, address):
        ...
        c = (address >> 16) & 0xFF
        d = (address >> 24) & 0xFF
        self._device.ctrl_transfer(0x21, int(Request.DNLOAD), 0, 0, [0x41, a, b, c, d])
        # time.sleep(0.5);
        self.get_status()  # this changes state
        status = self.get_status()  # this gets the status
        if status[2] == State.dfuDNLOAD_IDLE:
    ...
    def md380_custom(self, a, b):
        ...
        a &= 0xFF
        b &= 0xFF
        self._device.ctrl_transfer(0x21, int(Request.DNLOAD), 0, 0, [a, b])
        self.get_status()  # this changes state
        time.sleep(0.1)
        status = self.get_status()  # this gets the status
        if status[2] == State.dfuDNLOAD_IDLE:
