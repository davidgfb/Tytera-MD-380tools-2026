#!/usr/bin/env python3
"""Unwrap MD-380 SFUN-wrapped firmware using the XOR key from md380_fw.py.

TYT firmware files come wrapped in a SFUN container with header
"OutSecurityBin..." followed by XOR-encrypted application code.
The MD-380 uses a fixed 1024-byte cyclic XOR key to encrypt/decrypt.

Usage:
    python unwrap_md380.py input.bin output.bin

References:
    md380tools/md380_fw.py MD380FW.key (lines 118-182)
    md380tools/md380_fw.py MD380FW.header_fmt (line 196)

Tested with:
    firmware-2026-01-23-NoGPS.bin (995,840 bytes)
    -> firmware-2026-01-23-NoGPS-UNWRAPPED.bin (995,328 bytes)

Author: opencode session 2026-09-17
License: Public Domain / CC0
"""
import struct
import sys

# XOR key from md380tools/md380_fw.py MD380FW.key (lines 118-182)
KEY = (
...
# MD380FW header format from md380_fw.py line 196.
# Note: MD2017FW uses a DIFFERENT format (`<16s9s7s16s33s43s8sLLL112s`)
# and will fail to unwrap correctly. Use this one for MD-380.
...
def unwrap(in_path: str, out_path: str) -> None:
    """Unwrap a TYT MD-380 firmware file."""
    with open(in_path, 'rb') as f:
        img = f.read()

    if not img.startswith(b'OutSecurityBin'):
        raise ValueError(
            f"Not a TYT firmware file (bad magic): {img[:16]!r}"
        )

    hdr = struct.unpack(HEADER_FMT, img[:256])
    base_addr = int(hdr[6])
    app_len = int(hdr[7])
    rsrc_size = int(hdr[8])

    print(f"Magic:        {hdr[0][:14].decode('ascii', errors='replace')}")
    print(f"Base addr:    0x{base_addr:08X}")
    print(f"App length:   {app_len} bytes (0x{app_len:X})")
    print(f"Resource sz:  {rsrc_size} bytes (0x{rsrc_size:X})")

    if not (0x08000000 <= base_addr < 0x08200000):
        raise ValueError(f"Base address out of range: 0x{base_addr:08X}")
    if app_len != len(img) - 512:
        raise ValueError(
            f"App length mismatch: header says {app_len}, "
            f"file has {len(img) - 512}"
        )

    app = xor_cyclic(img[256:256 + app_len], KEY)
    print(f"First 16 bytes of unwrapped: {app[:16].hex()}")
    print(f"  Expected STM32 SP+Reset for MD-380: 70f10120 01af0908 ...")

    with open(out_path, 'wb') as f:
        f.write(app)
    print(f"\nWrote {len(app)} bytes to {out_path}")
